dynamicScriptByOS(); // 偵測哪個平台 , 但現在都走 pwa plugin 這隻js
...
function getPCC() {
    ...
    var kind = $("#selListKind").val(); // 哪個計畫？
    var amount = parseInt($("#selListAmount").val()); // 預算大小
    var keyword = $("#listSearch").val(); // 關鍵字
...
//    console.log(sqlcmd);  // sql.js
    var temp = function(){return sqlcmd;};
    db.transaction( // send sqlcmd to server (https://cordova.apache.org/docs/zh-cn/latest/cordova/storage/sqltransaction/sqltransaction.html)
        function (tx) {
            tx.executeSql(sqlcmd, [], function (tx2, res) {
                for (var i = 0; i < res.rows.length; i++) { // 尋覽抓回來的 資料? 清單?
                    ...
                    var state = res.rows.item(i).ExecuteState;
                        ...
                        ExecuteState = 5;
                    $("#item-" + ExecuteState).append(template); // 先分類 progress

                    var county = res.rows.item(i).CountyTown;
                    if (res.rows.item(i).CountyTown.length > 3)
                        county = res.rows.item(i).CountyTown.substring(3, 6); // 取得鄉鎮 ( 嘉義縣 XX鄉 )

                    var progress = res.rows.item(i).ActTotalProgress;  // 計算工程進度
                    ...
                    $("div[class='List PCC'] > label").html(String(res.rows.item(i).ActualBidAwardDate.substring(0, 3)) + "年度/" + county);  // 109年度 / XX鄉
                    $("div[class='List PCC'] > span").html(res.rows.item(i).ProjectName); // 專案名稱
                    $("div[class='List PCC'] > div > span").eq(0).html(progress); // 進度
                    $("div[class='List PCC'] > div > font").eq(0).html(nowProgress); // 進度狀態 : 超前 , 延後 ...
                    $("div[class='List PCC'] > div > span").eq(1).html(BidAwardAmount); // 預算 ??? 元
...
function getAnalysisA() {
    ...
    if (sDate != "" && eDate != "") {
        ...
        //where += "          (ActualBidAwardDate >= 1041001 and ActualBidAwardDate <= " + eDate + ") ";
        //where += "    and   (ProjectNo in (select ProjectNo from ENG_PCC_PROGRESS outerEng where ( (ExecuteState !='驗收完成' and ExecuteState !='保固中' and ExecuteState !='已結案' and ProjectNo not like'" + yearMin + "%') ";
        //where += "                                                                                  or  ";
        //where += "                                                                                  ((ExecuteState='驗收完成' or ExecuteState='保固中' or ExecuteState='已結案') and ProgYear='" + yearMin + "') ";
        //where += "                                                                               ) ";
        //where += "                                                                              and ProgYear = (select ProgYear from ENG_PCC_PROGRESS where ProjectNo=outerEng.ProjectNo order by ProgYear desc, ProgMonth desc limit 1) ";
        //where += "                                                                              and ProgMonth = (select ProgMonth from ENG_PCC_PROGRESS where ProjectNo=outerEng.ProjectNo order by ProgYear desc, ProgMonth desc limit 1))) ";
        where += "    ) ";
    }
    ...
    //sqlcmd += "union SELECT ProjectNo, AnnounceBudget, BidAwardAmount FROM ENG_PCC_BASIC where  ActualBidAwardDate like '" + yearMin + "%' and  length(ActualBidAwardDate) >0";
    sqlcmd += ")";
...
function getAnalysisB() {
    ...
    //sqlcmd += "union select BidAwardAmount FROM ENG_PCC_BASIC where  ActualBidAwardDate like '" + yearMin + "%' and  length(ActualBidAwardDate) >0";
...
function getAnalysisC() {
    ...
    //sqlcmd += "union select ProjectKind, BidAwardAmount FROM ENG_PCC_BASIC where  ActualBidAwardDate like '" + yearMin + "%' and  length(ActualBidAwardDate) >0";
